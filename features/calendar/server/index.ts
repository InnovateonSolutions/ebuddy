...
import { getGoogleCalendarEvents } from '@/features/calendar/server/google'
import { getMicrosoftCalendarEvents } from '@/features/calendar/server/microsoft'
  ...
  try {
    if (token.provider === 'GOOGLE') {
      ...
      if (result.newToken) {
        try {
          ...
          console.error('calendar.token_persist_failed', { provider: 'GOOGLE', error: e instanceof Error ? e.message : String(e) })
        }
      }
    ...
    if (result.newToken) {
      try {
        ...
        console.error('calendar.token_persist_failed', { provider: 'MICROSOFT', error: e instanceof Error ? e.message : String(e) })
      }
    }
    return result.events
  } catch (error) {
